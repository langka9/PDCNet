...
class Visualizer():
    def __init__(self, opt):
        ...
        self.summary_writer = SummaryWriter(log_dir=opt.run_dir)
    ...
    def summary_current_distribution(self, distribution, epoch):
        for name, value in distribution.items():
            self.summary_writer.add_histogram(name, value, epoch)
    ...
    def print_current_errors(self, epoch, i, errors, t):
        ...
        for k, v in errors.items():
            self.summary_writer.add_scalar(k, v, i)
            message += '%s: %.3f ' % (k, v)
    ...
    # summary the val image to tensorboard
    def summary_current_results(self, visuals, epoch):
        batchSize = self.opt.batchSize
        for label, image_numpy in visuals.items():
            self.summary_writer.add_image(f'img_{label}_iter_{epoch}', image_numpy, epoch, dataformats='HWC')
